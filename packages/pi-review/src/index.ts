 ...
 * - If a REVIEW.md file exists at the project root, its contents are appended
...
Below are default guidelines for determining what to flag. These are not the final word — if you encounter more specific instructions elsewhere (in a developer message, user message, or the project review file, those override these general instructions.
...
async function loadProjectReviewInstructions(
  cwd: string,
): Promise<string | null> {
  const reviewPath = path.join(path.resolve(cwd), "REVIEW.md")
  const stats = await fs.stat(reviewPath).catch(() => null)
...
export default function reviewExtension(pi: ExtensionAPI) {
  ...
  async function showBranchSelector(
    ...
    const sortedBranches = candidateBranches.toSorted((a, b) => {
  ...
  async function executeReview(
    ...
    const projectReviewInstructions = await loadProjectReviewInstructions(
      ctx.cwd,
    )
