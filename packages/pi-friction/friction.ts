...
const frictionSchema = Type.Object(
  {
    thought: Type.String({
      description: "Workflow-friction feedback text.",
    }),
...
export default function ventExtension(pi: ExtensionAPI) {
  pi.registerTool({
    name: "vent",
    label: "vent",
    description: "Append workflow-friction feedback to .pi/FRICTION.md.",
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      const frictionPath = resolve(ctx.cwd, ".pi", "FRICTION.md")
      const now = new Date()
      const timestamp =
        [
      ...
      return withFileMutationQueue(frictionPath, async () => {
        await mkdir(resolve(ctx.cwd, ".pi"), { recursive: true })

        if (!(await fileExists(frictionPath))) {
          await writeFile(frictionPath, heading, "utf8")
        }
        await appendFile(frictionPath, entry, "utf8")

        return {
          content: [
            {
              type: "text" as const,
              text: `Appended friction entry to .pi/FRICTION.md (${timestamp}).`,
            },
          ],
          details: { path: ".pi/FRICTION.md", timestamp, trigger, thought },
        }
      })
    },
    ...
    renderResult(result, { expanded }, theme) {
      ...
      let text = `${theme.fg("success", "✓")} wrote ${theme.fg("accent", ".pi/FRICTION.md")} ${theme.fg("dim", timestamp)}`
